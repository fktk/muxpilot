"""Tests for muxpilot.controllers.PaneTitleManager — in-memory overlay only."""
...
def make_node_data(node_type: str, session_name="work", window_index=0, pane_index=0):
    """Create node_data tuple for PaneTitleManager.start()."""
    session = SessionInfo(
...
class TestPaneTitleManagerPaneTitle:
    """PaneTitleManager sets tmux pane_title directly via TmuxClient."""

    def test_start_returns_pane_title(self) -> None:
        from unittest.mock import MagicMock
        client = MagicMock()
        ctrl = PaneTitleManager(client)
