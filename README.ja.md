...
- **🌲 階層ツリー表示**: tmux の `Session → Window → Pane` 構造をツリー表示します。
...
- **👀 ステータス監視**: 各ペインの出力を定期的にポーリングし、以下の4種類のステータスアイコンを自動付与します。
  ...
  | アイコン | ステータス | 検出条件 |
  |:---:|---|---|
  | 🚶/🏃 | ACTIVE | ペインの出力が変化した、またはプロンプト・エラー・アイドル以外の状態（コマンド実行中・ログ出力中など） |
  | ✋ | WAITING | 最終行がプロンプトパターンに一致している（ユーザーの入力待ち） |
  | 🚨 | ERROR | 直近10行にエラーパターン（`Traceback`, `Error:`, `FAILED` 等）が検出された |
  | 🛌 | IDLE | 設定した閾値以上、出力の変化がない |

  判定は **ERROR → WAITING → IDLE → ACTIVE** の優先順で行われます。  
  また、一度 `ACTIVE` から別の状態に遷移すると、次の出力が来るまでその状態を維持します（ヒステリシス）。

- **🏷️ ノード名の変更**: `n` キーで選択中のセッション・ウィンドウ・ペインに名前を付けられます。ペインの場合は tmux のネイティブなペインタイトルを変更するため、他の tmux クライアントでも反映されます。
- **📋 詳細パネル**: 選択中のペインで実行されているコマンド、フルコマンドライン、現在のディレクトリ、Git リポジトリ名とブランチ、ステータス、アイドル時間、直近の出力行などの詳細情報を表示。
...
sidebar_hide_threshold = 80  # 端末幅がこの値以下のとき詳細パネルを非表示

[notifications]
poll_errors = true                     # tmux ポーリング失敗時にトーストを表示
notification_cooldown = 5.0            # FIFO 経由で WAITING を強制した後の維持時間（秒）
waiting_trigger_pattern = "WAITING"    # FIFO 経由で WAITING ステータスを強制する正規表現

[logging]
level = "INFO"  # DEBUG, INFO, WARNING, ERROR（~/.config/muxpilot/muxpilot.log 用）
```
...
- `sidebar_hide_threshold`: 端末幅がこの値以下のとき詳細パネルを非表示にします。`0` で無効化。
- `poll_errors`: `false` にすると「tmux poll failed」通知を抑制します。
- `notification_cooldown`: 通知で `WAITING` を強制した後、その状態を維持する秒数。
- `waiting_trigger_pattern`: 外部 FIFO メッセージ内にペイン ID（例: `%1`）とこのパターンの両方が含まれる場合、そのペインを `WAITING` ステータスに強制します。
- `level`: muxpilot のログファイル（`~/.config/muxpilot/muxpilot.log`）用のログレベル。
...
muxpilot は画面右下にトースト通知を表示します。外部からの FIFO メッセージや、ポーリングエラー時に表示されます。
